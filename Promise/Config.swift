...
public enum Config {
    ...
    static var kakaoLocalSearchApiUrl: URL = {
        guard let baseURL = Config.infoDictionary[Keys.kakaoLocalSearchApiUrl.rawValue] as? String else {
            fatalError("kakao API URL not set in plist")
        }
        
        guard let url = URL(string: baseURL) else {
            fatalError("kakao API URL is invalid")
        }
    ...
    static let kakaoRestAppKey: String = {
        guard let kakaoRestAppKey = Config.infoDictionary[Keys.kakaoRestAppKey.rawValue] as? String else {
            fatalError("KAKAO REST APP KEY not set in plist")
        }
